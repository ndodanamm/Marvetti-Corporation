    ...
    <div className="animate-in fade-in duration-500 pb-32">
      ...
      <section className="relative pt-40 pb-32 overflow-hidden bg-slate-950">
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          <button 
            onClick={onBack}
            className="group flex items-center gap-3 text-brand-400 font-black uppercase text-[10px] tracking-[0.3em] mb-12 hover:text-white transition-colors"
          >
          ...
          <div className="max-w-7xl mx-auto px-4 relative z-10">
            <div className="flex flex-col md:flex-row gap-16 items-center">
              <div className="md:w-1/2">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-brand-500/10 text-brand-400 text-[9px] font-black uppercase tracking-widest mb-6 border border-brand-500/20">
                  <TrendingUp className="w-3 h-3" /> Measured Efficiency
                </div>
                <h2 className="text-3xl font-black text-white uppercase tracking-tighter mb-4">
                  Operational <span className="text-brand-500">Impact</span> Statistics
                </h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
                  {businessAdminCaseStudy.stats.map((stat, idx) => (
                    <div key={idx} className="p-6 rounded-3xl bg-white/5 border border-white/10 group hover:border-brand-500/50 transition-all shadow-xl">
                      <div className="text-3xl font-black text-white tracking-tighter mb-2 group-hover:text-brand-400 transition-colors">{stat.value}</div>
              ...
              <div className="md:w-1/2 grid grid-cols-2 gap-4">
                <div className="p-8 rounded-[3rem] bg-brand-600 text-white flex flex-col justify-between aspect-square group">
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid lg:grid-cols-3 gap-20">
            <div className="lg:col-span-2 space-y-20">
              <div>
                <h2 className="text-xs font-black text-brand-600 uppercase tracking-[0.4em] mb-8">Service Overview</h2>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {[
                    ...
                    <div key={skill} className="flex items-center gap-4 p-6 rounded-3xl bg-slate-50 border border-slate-100">
                      <div className="w-8 h-8 rounded-xl bg-slate-600 text-white flex items-center justify-center shrink-0">
            ...
            <div className="space-y-12">
              <div className="p-10 rounded-[3rem] bg-slate-950 text-white shadow-2xl">
                <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-brand-400 mb-10">Pillar Stack</h3>
                ...
                <button 
                  onClick={onOrder}
                  className="w-full py-5 bg-brand-600 hover:bg-brand-700 text-white rounded-2xl flex items-center justify-center gap-3 font-black uppercase tracking-widest text-xs shadow-xl shadow-brand-900/20 transition-all mb-4"
                >
              ...
              <div className="p-10 rounded-[3rem] border border-slate-100 bg-slate-50/50">
                ...
                <ul className="space-y-4">
                  <li className="text-sm font-medium text-slate-500 flex items-center gap-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-slate-400"></div> 100% Digital Workflow
                  </li>
                  <li className="text-sm font-medium text-slate-500 flex items-center gap-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-slate-400"></div> Real-time Progress Tracking
                  </li>
                  <li className="text-sm font-medium text-slate-500 flex items-center gap-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-slate-400"></div> Post-Delivery Support
                  </li>
                </ul>
              </div>
            </div>
      ...
      <section className="py-32 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-20">
            <h2 className="text-xs font-black text-brand-600 uppercase tracking-[0.4em] mb-4">Investment Tiers</h2>
            <h3 className="text-4xl md:text-5xl font-black text-slate-950 tracking-tighter">Choose Your Package</h3>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
             {service.packages.map((pkg, idx) => (
                <div key={idx} className={`p-12 rounded-[3.5rem] bg-white border transition-all duration-500 flex flex-col ${idx === 1 ? 'border-brand-600 shadow-2xl scale-105 z-10' : 'border-slate-200 hover:border-brand-200 shadow-lg'}`}>
                   {idx === 1 && (
                     <span className="bg-brand-600 text-white text-[8px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full w-fit mb-8 shadow-lg">Most Popular Choice</span>
                   )}
                   ...
                   <button 
                      onClick={onOrder}
                      className="w-full py-6 bg-slate-950 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-brand-600 transition-all flex items-center justify-center gap-3 group/btn"
                   >
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center justify-between mb-16 gap-6">
            <div className="text-center md:text-left">
               <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-50 text-slate-600 text-[9px] font-black uppercase tracking-widest mb-4 border border-slate-100">
                 <Sparkles className="w-3 h-3 text-brand-500" /> Enhance Your Project
               </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {upsells.map((upsell) => (
              <div key={upsell.id} className="group p-10 rounded-[3rem] bg-slate-50 border border-slate-100 hover:bg-white hover:border-slate-200 transition-all duration-500 hover:shadow-xl">
                 <div className="flex justify-between items-start mb-6">
                    <div className="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-slate-700 shadow-sm group-hover:scale-110 transition-transform">
                      {upsell.icon}
                    </div>
                    <div className="font-inter font-black text-slate-950 tracking-tighter">{upsell.price}</div>
                 </div>
                 <h4 className="text-md font-black text-slate-900 mb-3 tracking-tight group-hover:text-slate-950 transition-colors">{upsell.title}</h4>
                 ...
                 <div className="flex items-center justify-between pt-6 border-t border-slate-200/50">
                    ...
                    <button 
                      onClick={() => onOrder()} 
                      className="text-[10px] font-black uppercase tracking-widest text-slate-600 hover:text-brand-600 transition-colors"
                    >
                      Add to Plan +
                    </button>
                 </div>
